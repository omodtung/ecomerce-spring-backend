...
import com.shop.ecommerceGo.domain.Product_;
import com.shop.ecommerceGo.domain.User;
import com.shop.ecommerceGo.domain.dto.*;
...
public class ItemController {
  ...
  public String getCheckOutPage(@ModelAttribute("cart") Cart cart) {
    // truyen Card cha chua nhieu mang cardDetail(hay con goi la nhieu product ben trong nay)
    List<CartDetail> cartDetails = cart == null
      ? new ArrayList<CartDetail>()
      : cart.getCartDetails();
  ...
  public String getProductPage(
    ...
    if (
      ...
      if (sort.equals("gia-tang-dan")) {
        pageable =
          PageRequest.of(page - 1, 10, Sort.by(Product_.PRICE).ascending());
      } else if (sort.equals("gia-giam-dan")) {
        pageable =
          PageRequest.of(page - 1, 10, Sort.by(Product_.PRICE).descending());
      }
    }
    Page<Product> prs =
      this.productService.fetchProductsByPage(pageable, productCriteriaDTO);
    List<Product> products = prs.getContent().size() > 0
      ? prs.getContent()
